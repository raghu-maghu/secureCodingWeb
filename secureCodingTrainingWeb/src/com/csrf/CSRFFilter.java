...
//	private final static Logger LOG = LoggerFactory.getLogger(CSRFFilter.class);
	...
	private void doFilterInternal(HttpServletRequest req, HttpServletResponse resp, FilterChain chain) throws IOException, ServletException {
		if (!req.getMethod().equals("POST")) {
			...
			System.out.println("new csrf token generated: {} path: {}" + token + req.getServletPath());
		...
		if (csrfToken == null) {
			System.out.println("csrf token not found in POST request: {}"+ req.getServletPath());
			if (!resp.isCommitted()) {
				resp.sendError(400);
			}
			return;
		...
		for (Cookie curCookie : req.getCookies()) {
			if (curCookie.getName().equals(csrfTokenName)) {
				if (curCookie.getValue().equals(csrfToken)) {
					...
					System.out.println("mismatched csrf token. expected: {} received: {} path: {}");
					if (!resp.isCommitted()) {
						resp.sendError(400);
					}
					return;
		...
		System.out.println("csrf cookie not found at: {}"+ req.getServletPath());
		if (!resp.isCommitted()) {
			resp.sendError(400);
		}
	}
